...
| 19 | quota-cascade-prevention | **Queued, draft.** A doer that 429s today still fires checker + triage on its empty diff, often quota'ing the next CLI in turn — 2-3× extra agent calls per quota-failed attempt. Plan 19A: detect the rate-limit class on agent-call return, hold the task in `doing_subtask` instead of transitioning, don't burn attempt budget, back off until reset. Hard constraint: NO cost-to-subscription approximation (tokens-per-bucket varies 5× peak/off-peak, 2× weekend/weekday, plus provider promotions/load — local accounting is wrong by an unbounded factor). Real-signal monitoring only, à la codexbar. | — |
