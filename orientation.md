...
- **Agent layer (plan 38)**:
  ...
  - `quikode/agents/json_protocol.py` — `JsonAgentTransport` Protocol, `JsonOutputAgent`, `WritesFilesAgent`, `JsonAgentResult`. Pydantic validates every role payload. Both cli-native transports and client-side transports get one structured repair prompt if the returned object fails the role schema.
