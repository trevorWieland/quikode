...
- **Agent layer (plan 38)**:
  ...
  - `quikode/agents/json_protocol.py` — `JsonAgentTransport` Protocol, `JsonOutputAgent`, `WritesFilesAgent`, `JsonAgentResult`. Pydantic validates every role payload. Both cli-native transports and client-side transports get one structured repair prompt if the returned object fails the role schema. Client-side transports may recover a balanced, schema-valid JSON object from surrounding provider prose.
...
**Architectural rule (plan 38):** Every decision-making structured-output agent runs in JSON output mode. NO regex on agent stdout. NO `extract_json`-shaped heuristics. Either CLI-native schema enforcement (claude `--json-schema`, codex `--output-schema`) or pydantic validation + structured re-prompt-once. Client-side transports may recover a balanced JSON object from provider prose only if that object validates against the exact role schema. The role-binding axis is MODEL only — `cfg.<role>_cli` does not exist; the CLI is invisible to the role.
...
**Project bootstrap-command obligation (plan 55).** When a workspace opts into `cfg.audit_fresh_container=true`, the project must own a `cfg.audit_bootstrap_command` that produces clean state end-to-end — `pnpm install --frozen-lockfile && just regenerate-all`, `cargo build --workspace && cargo xtask codegen`, etc. The command runs inside a fresh container at `/workspace` with a 15-minute timeout; whatever sequence GitHub Actions runs to materialize a usable workspace from a fresh checkout is what the bootstrap command should mirror. If the bootstrap is incomplete (skips a regeneration step a CI workflow runs), plan 53's `cannot_reproduce` signal will still fire — that's the diagnostic that the bootstrap needs another step. Iterate the bootstrap command project-side; quikode does not prescribe what "clean state" means.
...
- **Launch config is fail-fast.** `qk run` / `qk daemon start` validates runtime-critical config before workers start: repo path, DAG path, local-CI command, loaded standards profiles, and loaded architecture docs. Missing standards/architecture corpora are startup blockers, never pre-PR runtime findings. Persisted per-task `evaluation_contract.json` files are also refreshed if they were created while audit corpora were empty and the current launch config can now load docs. Fresh tanren workspaces should point `standards_profiles_dir` at `<repo>/profiles`, set `standards_profiles = ["rust-cargo"]`, and point `architecture_docs_dir` at `<repo>/docs/architecture`.
- **Z-99 uses the full spec gate.** The injected `Z-99-stabilize-spec-gate` subtask is the only subtask whose objective gate runs `local_ci_command` (`just ci` for tanren) instead of `subtask_check_command` (`just check`). A green Z-99 without objective `just ci` output is not authoritative; the pre-PR local-CI stage remains the source of truth.
- **Behavior witnesses must execute, but can recover from missing DAG commands.** The scoped witness runner first uses `command` / `witness_command` / command-like `witnesses[]` from the DAG, then reloads the current worktree DAG, then as a last resort selects a behavior-looking command from the validated doer envelope. The envelope is not trusted as proof; it only supplies a command that quikode then executes and records with rc/output.
- **Playwright browser cache is host-backed, but OS deps live in the image.** Task containers mount `cfg.playwright_cache_dir` at `/home/dev/.cache/ms-playwright`; the quikode dev image installs Chromium system libraries via Playwright's `install-deps chromium`. If every fresh container fails `just ci` at `web-storybook-test` with "Executable doesn't exist ... chromium_headless_shell", populate the cache once from the tanren image with `pnpm --filter @tanren/web exec playwright install chromium`. If Chromium exists but fails on `libglib-2.0.so.0` or similar shared libraries, rebuild/reinstall the quikode image before blaming a task branch.
