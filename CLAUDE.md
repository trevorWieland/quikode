...
- Run `python -m pytest tests/ -q` after any change touching `quikode/`. **784 tests**; runs in ~60s.
...
- v3.6 pre-PR pipeline: between `_final_check_loop` and `_open_pr`, the worker runs a 4-stage gate when `cfg.pre_pr_pipeline_enabled=True` (default): local-CI (`just ci`) → rubric (codex 6 categories, threshold 7) → standards (claude-opus reads `cfg.pre_pr_standards_profile_globs` + branch diff) → behavior (codex verifies each `expected_evidence`). All four run per cycle; failures merge into a triage bundle → `_run_fixup_round(kind="fixup-pre-pr-audit")` → subtask loop → re-enter pipeline. Cap via `cfg.pre_pr_audit_max_cycles=3`. New states: `LOCAL_CI_CHECKING`, `PRE_PR_AUDITING`, `PRE_PR_TRIAGING`. Set `pre_pr_pipeline_enabled=false` to fall back to the legacy direct-to-PR path.
- v3.6 BLOCKED-as-bug forensics: every transition INTO `BLOCKED` (from non-BLOCKED) auto-captures a comprehensive snapshot via `Store.capture_block_forensics`: retry-cause histogram, last 5 checker outputs, last 5 triage notes, last 3 progress-check verdicts, peak rss, last 20 state transitions, subtask state distribution. Persisted on `tasks.block_forensics`. Surfaced in `quikode unblock <id>`. The framing reflects the operator-stated truth: a 30-retry BLOCK is a quikode failure to detect/work around something earlier, not a graceful gate. The retry-classifier capture bug (rc=None, empty stderr → all signatures `rc=?`) was also fixed so the histogram has real data.
