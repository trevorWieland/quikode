...
from . import config_descriptions as desc
...
class Config(BaseModel):
    ...
    subtask_doer_timeout_s: int = Field(
        ...
        ge=60,
        le=14400,
        description=desc.SUBTASK_DOER_TIMEOUT,
    )
    subtask_checker_timeout_s: int = Field(
        default=900,
        ge=30,
        le=3600,
        description=desc.SUBTASK_CHECKER_TIMEOUT,
    )
    ...
    fixup_planner_timeout_s: int = Field(
        ...
        ge=120,
        le=3600,
        description=desc.FIXUP_PLANNER_TIMEOUT,
    )
    ...
    fixup_planner_output_retries: int = Field(
        ...
        ge=1,
        le=20,
        description=desc.FIXUP_PLANNER_OUTPUT_RETRIES,
    )
    ...
    review_ready_settle_s: int = Field(
        ...
        ge=0,
        le=14400,
        description=desc.REVIEW_READY_SETTLE,
    )
    ...
    conflict_resolver_max_iterations: int = Field(
        ...
        ge=1,
        le=20,
        description=desc.CONFLICT_RESOLVER_MAX_ITERATIONS,
    )
    rebase_max_attempts: int = Field(
        default=2,
        ge=1,
        le=10,
        description=desc.REBASE_MAX_ATTEMPTS,
    )
    ...
    subtask_witness_timeout_seconds: int = Field(
        ...
        ge=1,
        le=600,
        description=desc.SUBTASK_WITNESS_TIMEOUT,
    )
