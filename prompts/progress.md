...
{
  "verdict": "progressing" | "flatline" | "uncertain",
  "rationale": "one sentence; cite specific evidence from the attempt history"
}

## Heuristics
- "flatline" = the same root cause repeats across the last 2-3 attempts AND triage notes don't show new strategy. Or: triage notes propose the same fix pattern repeatedly with no convergence.
...
If the triage notes for the last 2+ attempts contain phrases like "blocked on owner", "out-of-scope", "pre-existing failure", "upstream fix needed", or otherwise claim the root cause is something the doer "shouldn't" fix, that is **always flatline**. The orchestrator's contract is that this branch's task owns every commit on it; there is no separate upstream owner. A subtask that keeps deferring the real fix to "someone else" is by definition not making progress on the acceptance criteria. Return `flatline` even if the surface phrasing of each triage round looks slightly different.
