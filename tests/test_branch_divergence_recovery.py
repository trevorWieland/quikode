...
def test_diverged_but_mergeable_falls_through(tmp_path):
    """Diverged (ahead > 0 AND behind > 0) but base_ref_sha still reachable
    → not a force-push. Today we fall through (return None) to legacy
    push-fail handling. Future: pull --rebase + conflict-resolver."""
    worker = _build_worker(tmp_path)
    ...
    assert outcome is None
    # Task still in DOING_SUBTASK — no BLOCK.
    assert worker.store.get("R-001")["state"] == State.DOING_SUBTASK.value
    worker.store.conn.close()
