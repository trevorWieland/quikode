...
def test_load_config_wires_subtask_cannot_reproduce_stop_loss_count(tmp_path):
    """Plan 53: the new `subtask_cannot_reproduce_stop_loss_count`
    Field must be readable from the workspace toml. Plan 50's
    orphan-audit warning would otherwise surface this as a silent
    override."""
    _scaffold_workspace(
        tmp_path,
        toml_body=(
            ...
            'repo_path = "{repo}"\n'
            'dag_path = "{dag}"\n'
            "subtask_cannot_reproduce_stop_loss_count = 4\n"
        ),
    )
    cfg = load_config(tmp_path)
    assert cfg.subtask_cannot_reproduce_stop_loss_count == 4
...
def test_load_config_reads_pre_pr_budget_and_release_valve_knobs(tmp_path):
    _scaffold_workspace(
        ...
        toml_body=(
            ...
            "release_valve_after_cycles = 6\n"
            'release_valve_defer_stages = ["standards", "architecture"]\n'
    ...
    assert cfg.release_valve_after_cycles == 6
    assert cfg.release_valve_defer_stages == ["standards", "architecture"]
